...
class ALogDataBroker
{
 public:
  ...
  ALogDataBroker(const ALogDataBroker&);
  ALogDataBroker& operator=(const ALogDataBroker&);
  ALogDataBroker(ALogDataBroker&&) noexcept = default;
  ALogDataBroker& operator=(ALogDataBroker&&) noexcept = default;
 ...
 protected:
  ...
  std::vector<std::unique_ptr<SplitHandler> > m_splitters; // addALogFile()
  std::vector<bool>         m_is_sqlite;        // addALogFile()
