...
ALogDataBroker::ALogDataBroker(const ALogDataBroker& other)
{
  *this = other;
}
...
ALogDataBroker& ALogDataBroker::operator=(const ALogDataBroker& other)
{
  if(this == &other)
    return(*this);

  m_alog_files   = other.m_alog_files;
  m_is_sqlite    = other.m_is_sqlite;
  m_summ_files   = other.m_summ_files;
  m_base_dirs    = other.m_base_dirs;
  m_vnames       = other.m_vnames;
  m_vtypes       = other.m_vtypes;
  m_vcolors      = other.m_vcolors;
  m_vlengths     = other.m_vlengths;
  m_logstart     = other.m_logstart;
  m_logtmin      = other.m_logtmin;
  m_logtmax      = other.m_logtmax;
  m_logskew      = other.m_logskew;

  m_global_logstart = other.m_global_logstart;
  m_global_logtmin  = other.m_global_logtmin;
  m_global_logtmax  = other.m_global_logtmax;
  m_pruned_logtmin  = other.m_pruned_logtmin;
  m_pruned_logtmax  = other.m_pruned_logtmax;

  m_mix_vname    = other.m_mix_vname;
  m_mix_alog_file= other.m_mix_alog_file;
  m_mix_varname  = other.m_mix_varname;
  m_mix_vartype  = other.m_mix_vartype;
  m_mix_source   = other.m_mix_source;
  m_mix_alog_ix  = other.m_mix_alog_ix;

  m_bix_vname    = other.m_bix_vname;
  m_bix_bhvname  = other.m_bix_bhvname;
  m_bix_alog_ix  = other.m_bix_alog_ix;

  m_alix_vname   = other.m_alix_vname;
  m_alix_appname = other.m_alix_appname;
  m_alix_alog_ix = other.m_alix_alog_ix;

  m_region_info  = other.m_region_info;
  m_detached_pairs = other.m_detached_pairs;

  m_verbose      = other.m_verbose;
  m_progress     = other.m_progress;
  m_max_fileptrs = other.m_max_fileptrs;
  m_vqual        = other.m_vqual;

  m_splitters.clear();
  m_splitters.reserve(other.m_splitters.size());
  for(unsigned int i=0; i<other.m_splitters.size(); ++i) {
    if(other.m_splitters[i] && !m_is_sqlite[i]) {
      std::unique_ptr<SplitHandler> handler(new SplitHandler(m_alog_files[i]));
      handler->setProgress(m_progress);
      handler->setVerbose(m_verbose);
      handler->setMaxFilePtrCache(m_max_fileptrs);
      for(const auto& pair : m_detached_pairs)
        handler->addDetachedPair(pair);
      m_splitters.push_back(std::move(handler));
    }
    else
      m_splitters.push_back(std::unique_ptr<SplitHandler>());
  }
...
void ALogDataBroker::addALogFile(string alog_file)
{
  ...
  bool is_sqlite = strEnds(alog_file, ".sqlite");
  m_is_sqlite.push_back(is_sqlite);

  if(is_sqlite) {
    m_splitters.push_back(std::unique_ptr<SplitHandler>());
  }
  else {
    std::unique_ptr<SplitHandler> handler(new SplitHandler(alog_file));
    handler->setProgress(m_progress);
    handler->setVerbose(m_verbose);
    handler->setMaxFilePtrCache(m_max_fileptrs);
    m_splitters.push_back(std::move(handler));
  ...
  for(unsigned int i=0; i<m_alog_files.size(); i++) {
    if(m_is_sqlite[i]) {
      if(!okFileToRead(m_alog_files[i])) {
        cout << "file " << m_alog_files[i] << " cannot find or open." << endl;
        all_ok = false;
      }
    }
    else if(m_splitters[i])
...
bool ALogDataBroker::splitALogFiles()
{
  ...
  for(unsigned int i=0; i<vsize; i++) {
    string base_dir = m_alog_files[i];
    rbiteString(base_dir, '.');
    base_dir += "_alvtmp";
    ...
    if(m_is_sqlite[i]) {
      ...
      SQLiteALogLoader loader;
      loader.setVerbose(m_verbose);
      bool ok = loader.exportCache(m_alog_files[i], base_dir);
      if(!ok)
        all_ok = false;
    }
    else if(m_splitters[i]) {
    ...
    string summary_file = base_dir + "/summary.klog";
